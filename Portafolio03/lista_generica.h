...
// Constructor con tamaños por defecto, en este caso 4
template <typename TIPODATO>
...
Lista<TIPODATO>::Lista(const Lista<TIPODATO> &otra) {
	//cout << "Copiando una Cola desde otra..." << endl;
...
int Lista<TIPODATO>::tamano(){
	//cout << "Obteniendo el tamano de la Cola..." << endl;
	return this->cuenta;
}

template <typename TIPODATO>
void Lista<TIPODATO>::agrandar(){
	//cout << "Duplicando el tamano de la cola..." << endl;
	...
	for (int i = 0; i < cuenta; i++)
	     this->items[i] = temp[i];
	delete[] temp;
	//cout << "La cola tienen capacidad para" << this->capacidad << " elementos " << endl;
}

// insertar un elemto en un indice especifico
template <typename TIPODATO>
void  Lista<TIPODATO>::insertar(int indice,  TIPODATO item) {
	// Validar el indice
	if (indice < 0 || indice > this->cuenta) throw "Indice fuera de rango";

    // Agrandar la lista si el elemento noc abe 
	...
	//cout << "Incrementandola cuenta de elementos " << endl;
	this->cuenta++; 
}

// Agregar un elemnto añl final 
template <typename TIPODATO>
void Lista<TIPODATO>::adjuntar(TIPODATO item){
	this->insertar(this->cuenta, item);
}

// Valor del elemento en la posision indice
template <typename TIPODATO>
...
TIPODATO Lista<TIPODATO>::remover(int indice){
	...
	//cout << "Removimiento el elemento con indice " << indice << "-...." << endl;
